...
const createSchema = joi.object({
  ...
  refreshToken: joi.string().allow(null).allow(''),
  tokenExpirationTime: joi.date().options({ convert: true }).allow(null).allow(''),
...
let filterValidationSchema = joi.object({
  ...
  ...Object.fromEntries(
    keys.map(key => [key, joi.object({
      ...
      refreshToken: joi.alternatives().try(joi.array().items(),joi.string(),joi.object()),
      tokenExpirationTime: joi.alternatives().try(joi.array().items(),joi.date().options({ convert: true }),joi.object()),
